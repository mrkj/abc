...
##            abc('r &s_savetemp.aig'%f_name)
        ...
        if is_sat():
            ...
            print "No CEX found in %d frames"%bmc_depth()
            if reach_sw == 0:
                break
            else:
                continue
...
def scorr_constr():
    ...
    if ((na > 40000) or n_pos()>1):
        return Undecided_no_reduction
    print "RUNNING signal correspondence with constraints"
    f = 40000/na
    f = min(f,16)
    n_pos_before = n_pos()
...
##            return result
    ...
    if result == 0:
        ...
        result = 'UNDECIDED'
    print 'Additional time taken by BMC/Reachability = %f sec.'%(time.clock() - x)
    return result
